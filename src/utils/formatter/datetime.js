...
export const calculateTimezoneName = () => getTimezoneName(getTimezoneNew())

export const dateTimeToLocalTime = datetime => datetime
  && `${customFormat(datetime, 'HH:mm')} ${calculateTimezoneName(datetime)}`

export const datetimeToLocalWithTimezone = datetime => datetime
  && `${dayjs(datetime).format(commonDateTimeFormat)} ${calculateTimezoneName()}`
